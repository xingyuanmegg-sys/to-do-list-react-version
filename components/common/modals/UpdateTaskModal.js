...
const UpdateTaskModal = (props) => {
  ...
  useEffect(() => {
    if (props.isVisible && props.task) {
      ...
      if (props.task.dueDate) {
        setSelectedDate(new Date(props.task.dueDate));
        setHasDate(true);
      } else {
  ...
  const handleCancel = () => {
    if (
      taskName.trim() !== (props.task?.name || "") ||
      hasDate !== !!props.task?.dueDate ||
      (hasDate &&
        selectedDate &&
        props.task?.dueDate &&
        selectedDate.getTime() !== new Date(props.task.dueDate).getTime())
    ) {
      Alert.alert(
        "Unsaved Changes",
